...
function f6() {
    let out = '';
    let ikey = document.querySelector('.i-61').value;
    let ival = document.querySelector('.i-62').value;
...
// получать из i-7 ключ. Если такой ключ есть в a7 то выводить 1 в out-7, если нет - 0.
...
function f7() {
    let out = '';
    let ikey = document.querySelector('.i-7').value;
    if (ikey !== '') {
        if (a7[ikey] !== undefined) {
            out = '1';
        } else {
            out = '0';
        }
...
// Task 8
// Добавьте input .i-8. При нажатии b-8 выполняете функцию f8. Функция
// должна выводить значение в out-8,
// если ключ введенный в i-8 есть в массиве, если нет - 0.
...
function f8() {
    let out = '';
    let ikey = document.querySelector('.i-8').value;

    if (ikey !== '') {
        // поиск по массиву v
         if (a8[ikey] !== undefined) {
                out += a8[ikey];
            } else {
            out = '0';
            }
...
// значению в input.i-9. Вывод через пробел. Если значений - нет -
// то выводить пустую строку.
...
function f9() {
    let out = '';
    let ikey = document.querySelector('.i-9').value;

    for(let key in a9) {
        if (ikey == a9[key]) out += key + ' ';
    }
...
// Task 11
// При нажатии b-11 выполняете функцию f11. Функция должна получить ключ
// из i-11 и удалить запись из массива a11 с таким ключем. После этого вывести
// массив в out-11. Для вывода используйте функцию f5.
...
// из i-12 и удалить запись из массива a12 с таким значением. После этого
// вывести массив в out-12. Для вывода используйте функцию f5.
...
function f14() {
    ...
    document.querySelector('.out-14').innerHTML = el;
}
...
// Task 16
// При нажатии b-16 выполняете функцию f16. Функция должна в out-16
// выводить элементы name вложенных массивов в a16. Вывод через пробел.
...
// вывести станции метро из массива a18 той ветки, которую пользователь
// ввел в i-18. Вывод станций - через пробел. Если ветка не найдена выводите
// пустую строку.
...
// цвет ветки станции которую пользователь ввел в i-19. Пользователь может
// вводить текст как с большой, так и с маленькой буквы. Если ветка не
// найдена - выводите пустую строку.
    ...
    for (let key in a19){
        for (let i = 0; i < a19[key].length; i++){
            if (a19[key][i].toLowerCase() == inp){
                out += key
                console.log(a19[key][i].toLowerCase());
            }
        }
    }
...
function f20() {
    ...
    for (let k in a20){
        for (let i = 0; i < a20[k].length; i++) {
            for (let j = 0; j < a20[k][i][j].length; j++) {
                if (a20[k][i][1] === 2) {
                    out += a20[k][i][0] + ' ';
                }
            }
...
// не совсем ясен кейс проверки if (a18[inp] !== undefined)
// не совсем понятно правильно ли проверяет на пустую строку if (ikey !== '')
// проверяют ли эти две проверки на одно и то же
