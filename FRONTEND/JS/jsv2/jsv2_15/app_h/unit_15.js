...
// let s1 = new ...
    ...
    s1.add('b');
    s1.add('o');
    s1.add('h'); // не вывелся потому что не уникальный
    console.log(s1);

// Task 2
// При нажатии b-2 выполняете функцию f2. Функция должна добавить в набор
// s2 элементы, которые пользователь вводит в i-2. Функция должна выводить
// в консоль s2 после каждого добавления элемента.
...
const f2 = () => {
    ...
    s2.add(inp);
    inp = document.querySelector('.i-2').value = ''; // очистка инпута
    console.log(s2);
}
...
// Task 3
// При нажатии b-3 выполняете функцию f3. Функция должна удалить из набора s3
// строку, которую пользователь вводит в i-3. Функция должна выводить в консоль
// s3 после каждого удаления элемента.
...
const f3 = () => {
    ...
    s3.delete(inp);
    inp = document.querySelector('.i-3').value = '';
    console.log(s3);
}
...
// Task 4
// При нажатии b-4 выполняете функцию f4. Функция должна проверить
// наличие в наборе s4 строки введенной пользователем в i-4.
...
const f4 = () => {
    ...
    document.querySelector('.out-4').innerHTML = s4.has(inp);
...
const f6 = () => {
    let b = new Set(a6);
    document.querySelector('.out-6').innerHTML = b.size;
}
...
const f7 = () => {
    ...
    // если запихнуть inp.length в иф то ничего неотработает
    let inlen = inp.length;
    let sesiz = set.size;

    const ifunic = (inlen, sesiz) => {
        if (inlen == sesiz) {
            // console.log('нет повторяющихся, значение равно');
            return 1;
        } else {
            // console.log('есть повторяющиеся, значение не равно');
            return 0;
        }
        // если ты уникальный 1
    }

    if (inp != '') {
        if (sesiz <= 6 || !ifunic) {
            // console.log('меньше или равно 6 символам');
            out = 0;
        } else {
            // console.log('больше 6 символов');
            out = 1;
        }
    }
...
// Task 9
//  При нажатии b-9 выполняете функцию f9. Функция должна принимать набор
// our_set в качестве параметра, преобразовывать его в строку, причем после
// каждого символа строки должен быть пробел. Функция должна возвращать результирующую строку.
...
// массив a11 в набор. И выводить в консоль. Изучите вывод получившегося
// набора. Разберитесь почему так происходит.
...
// найти сколько раз каждый символ встречается в исходном массиве. Результат -
// в виде объекта типа { символ : количество, символ : количество }
